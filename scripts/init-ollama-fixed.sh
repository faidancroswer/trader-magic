...
for i in $(seq 1 60); do
  if curl -s http://ollama:11434/api/version > /dev/null 2>&1; then
    ...
    if curl -s http://ollama:11434/api/tags | grep -q "$MODEL"; then
      ...
      # Use a longer timeout for pulling the model
      curl --max-time 600 -X POST http://ollama:11434/api/pull -H "Content-Type: application/json" -d "{\"name\":\"$MODEL\"}"
      ...
      for j in $(seq 1 60); do
        ...
        echo "Waiting for model to be ready (attempt $j/60)"
        sleep 10
      done
